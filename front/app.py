...
if 'usd_rate' not in st.session_state:
    st.session_state.usd_rate = get_usd_rate()
...
if not raw_df.empty:
    ...
    df = df[(df['ROI'] > -15) & (df['ROI'] < 45)]

    st.sidebar.header("Управление")
    search = st.sidebar.text_input("Поиск скина")
    min_roi = st.sidebar.slider("Мин. ROI (%)", -5.0, 25.0, 0.0)
    min_v = st.sidebar.number_input("Мин. продаж (Vol)", value=1)

    if st.sidebar.button("Очистить кэш"):
        st.cache_data.clear()
        st.rerun()

    f_df = df[(df['ROI'] >= min_roi) & (df['Vol'] >= min_v)]
    if search:
        f_df = f_df[f_df['Название'].str.contains(search, case=False)]

    m1, m2, m3, m4 = st.columns(4)
    m1.metric("Связок в базе", f"{len(df):,}".replace(',', ' '))
    max_p = f_df['Прибыль'].max() if not f_df.empty else 0
    m2.metric("Максимальная чистая прибыль", f"{max_p:,.0f} ₽".replace(',', ' '))
    m3.metric("Курс USD", f"{st.session_state.usd_rate} ₽")

    total_checked_filtered = f_df['check_time'].notnull().sum()
    m4.metric("Проверено", f"{total_checked_filtered:,}".replace(',', ' '))

    col_table, col_panel = st.columns([2.5, 1])

    with col_table:
        f_df = f_df.sort_values("ROI", ascending=False)

        def format_status(row):
            if pd.isnull(row['check_time']):
                return "Ожидание"

            check_time = pd.to_datetime(row['check_time'])
            ...
            diff = (now_utc - check_time).total_seconds() / 60

            return "Актуальный" if diff < 45 else "Ожидание"
        ...
        selection = st.dataframe(
            ...
            column_config={
                ...
                "price_lis": st.column_config.NumberColumn("Lis ($)", format="$ %d"),
                "Vol": st.column_config.ProgressColumn("Ликвидность", min_value=0, max_value=300, format="%d"),
            },
    ...
    with col_panel:
        ...
        if len(selection.selection.rows) > 0:
            ...
            st.write(
                f"Закуп (с комиссией): **{sel_item['price_lis'] * st.session_state.usd_rate * 1.03:,.2f} ₽**".replace(
                    ',', ' '))
